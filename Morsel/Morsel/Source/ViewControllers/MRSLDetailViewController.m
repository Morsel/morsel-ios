...
@interface MRSLDetailViewController ()
...
@implementation MRSLDetailViewController
    ...
    if (_morsel && _morsel.post) {
        ...
        if ([_morselPost.morsels count] > 1) {
            ...
            for (MRSLMorsel *morsel in orderedMorselsArray) {
                MRSLDetailPanelViewController *morselDetailPanelVC = [[UIStoryboard morselDetailStoryboard] instantiateViewControllerWithIdentifier:@"sb_MorselDetailPanel"];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"seg_AddComment"]) {
        [[MRSLEventManager sharedManager] track:@"Tapped Add Comment"
                              properties:@{@"view": @"MRSLDetailViewController",
...
- (void)didUpdateCurrentPage:(NSUInteger)page {
    ...
    if (self.previousExists) {
        MRSLDetailPanelViewController *previousDetailPanelViewController = [self.swipeViewControllers objectAtIndex:self.previousPage];
        previousDetailPanelViewController.delegate = nil;
    }

    MRSLDetailPanelViewController *detailPanelViewController = [self.swipeViewControllers objectAtIndex:page];
